...
def session_to_dict(row):
    ...
    d['roi_pct'] = round(((d['cash_out'] - d['buy_in']) / d['buy_in']) * 100, 1) if d['buy_in'] else 0
    return d

@gambling_bp.route('/', methods=['GET'])
def get_sessions():
    db = get_db()
    sessions = db.execute('SELECT * FROM gambling_sessions ORDER BY date DESC, id DESC').fetchall()
    db.close()
...
def create_session():
    ...
    required = ['game_type', 'buy_in', 'cash_out', 'date']
    for f in required:
        if f not in data:
...
def update_session(session_id):
    data = request.get_json()
    db = get_db()
    existing = db.execute('SELECT * FROM gambling_sessions WHERE id = ?', (session_id,)).fetchone()
    if not existing:
        db.close()
    ...
    db.execute(f'UPDATE gambling_sessions SET {set_clause} WHERE id = ?', list(updates.values()) + [session_id])
    db.commit()
